...
```bash
stepfunctions-template$ sam build --use-container
```
...
```bash
stepfunctions-template$ sam logs -n StockCheckerFunction --stack-name stepfunctions-template --tail
```
...
```bash
stepfunctions-template$ pip install -r tests/requirements.txt --user
# unit test
stepfunctions-template$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
stepfunctions-template$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name stepfunctions-template
```
